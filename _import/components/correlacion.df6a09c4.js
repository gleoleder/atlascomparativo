...
// ── Calcular todo ─────────────────────────────────────────────
export function computeAll(xs, ys, lngs, lats, geoms) {
...
// ── Extraer pares desde features ya capturadas (sin viewport) ──
export function extraerPairesDeFeatures(features, campoA, campoB, campos) {
  const vacio = { xs: [], ys: [], lngs: [], lats: [], geoms: [] };
  if (!features || features.length === 0 || !campoA || !campoB) return vacio;

  const keyA = campos[campoA];
  const keyB = campos[campoB];
  if (!keyA || !keyB) return vacio;

  const xs = [], ys = [], lngs = [], lats = [], geoms = [];
